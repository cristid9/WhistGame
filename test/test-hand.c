...
{
    struct Hand *hand = hand_createHand();
    cut_assert_equal_int(NO_ERROR, hand_deleteHand(&hand));
...
void test_hand_addPlayer()
{
    ...
    for (int i = 1; i <= MAX_GAME_PLAYERS; i++) {
        ...
        cut_assert_equal_int(NO_ERROR, hand_addPlayer(hand, players[i]));
        cut_assert_equal_int(DUPLICATE, hand_addPlayer(hand, players[i]));
        int check = 0;
